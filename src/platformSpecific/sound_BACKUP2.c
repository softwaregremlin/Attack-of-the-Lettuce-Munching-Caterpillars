...
#define BACKGROUND_MUSIC_FILE_ARRAY_LENGTH 10
...
int g_soundBackgroundMusicVorbisFileCount = 0;
int g_soundBackgroundMusicVorbisFileHasPlayed[BACKGROUND_MUSIC_FILE_ARRAY_LENGTH];
...
void soundClearBackgroundMusicOgg(int index) {
	ov_clear(&g_soundBackgroundMusicOggFile[index]);
	g_soundBackgroundMusicVorbisFileCount--;
}

void soundUnloadMusic() {
	for(int mm=g_soundBackgroundMusicVorbisFileCount - 1; mm >=0; mm--) {
		ov_clear(&g_soundBackgroundMusicOggFile[mm]);
	}
}

int soundLoadBackgroundMusic(char *path) {
	if(g_soundBackgroundMusicVorbisFileCount >= BACKGROUND_MUSIC_FILE_ARRAY_LENGTH)  return 0;		
	g_soundBackgroundMusicVorbisFileCount++;
	g_soundBackgroundMusicVorbisFileHasPlayed[g_soundBackgroundMusicVorbisFileCount - 1] = 0;
	if(ov_fopen(path, &g_soundBackgroundMusicOggFile[g_soundBackgroundMusicVorbisFileCount - 1]) < 0) return 0;
	return g_soundBackgroundMusicVorbisFileCount;
 }
 
  // Background Music Playing
  
 void soundRewindIfAlreadyPlayed(int index) {
	// Sound being played for the first time
 	if(!g_soundBackgroundMusicVorbisFileHasPlayed[index]) {
		g_soundBackgroundMusicVorbisFileHasPlayed[index] = 1;
		return;
	}
	// Sound has already been played
	ov_pcm_seek(&g_soundBackgroundMusicOggFile[g_soundBackgroundMusicVorbisFileIndex], 0);	
 }


  void soundStartBackgroundMusic(int musicNumber, int repeats) {
    g_soundBackgroundMusicVorbisFileIndex = musicNumber - 1;
	soundRewindIfAlreadyPlayed(g_soundBackgroundMusicVorbisFileIndex);
	g_soundBackgroundMusicIsPlaying = 1;
	g_soundBackgroundMusicLoop = repeats;	
 }
  void soundStartBackgroundMusicLoop(int musicNumber) {	
	soundStartBackgroundMusic(musicNumber, 0);
 }
...
int SoundLoadBackgroundMusicX(const char* filename) {
	char folderAndFile[50] = "app0:data/sounds/";
	strcat(folderAndFile, filename);
	return soundLoadBackgroundMusic(folderAndFile);
}

int SoundLoadBackgroundMusic2(const char* filename) {
	char folderAndFile[50] = "app0:data/sounds/";
	strcat(folderAndFile, filename);
	return soundLoadBackgroundMusic(folderAndFile);
}

int SoundLoadSampleX(const char* filename) {
	char folderAndFile[50] = "app0:data/sounds/";
	strcat(folderAndFile, filename);
...
/*
void SoundStartBackgroundMusicLoop(int musicNumber)          { soundStartBackgroundMusicLoop(musicNumber);      }
void SoundStopBackgroundMusic()     		                 { soundStopBackgroundMusic();                      }
void SoundStartBackgroundMusic(int musicNumber, int repeats) { soundStartBackgroundMusic(musicNumber, repeats); }
void SoundPlaySample(int sampleNumber) 		                 { soundPlaySample(sampleNumber);                   }
void InitialiseSound() { soundInitialise(); }
*/

void SoundStartBackgroundMusicLoopX(int musicNumber) 		 {  }
void SoundStopBackgroundMusic()     		                 {  }
void SoundStartBackgroundMusicX(int musicNumber, int repeats) {  }
void SoundPlaySampleX(int sampleNumber) 		                 {  }
void InitialiseSoundX() {}
